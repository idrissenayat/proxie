    ...
    return (
        <div className="bg-zinc-900 border border-zinc-700/50 rounded-[2.5rem] overflow-hidden w-full max-w-md animate-in fade-in slide-in-from-bottom-6 duration-700 shadow-2xl">
                        ...
                        {data.services?.map((svc, i) => (
                            <div key={i} className="bg-zinc-800/30 rounded-2xl p-4 border border-zinc-800">
                                ...
                                <p className="text-zinc-400 text-xs mt-1">
                                    ${svc.price_min || svc.price}-${svc.price_max || svc.price} • {svc.duration_min || svc.duration}-{svc.duration_max || svc.duration} min
                                </p>
            ...
            <div className="p-8 bg-zinc-900 border-t border-zinc-800 flex flex-col gap-3">
                <button
                    onClick={onSubmit}
                    className="w-full h-14 bg-white text-black rounded-2xl font-black text-sm flex items-center justify-center hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-white/5 cursor-pointer"
                >
                ...
                <button
                    onClick={onEdit}
                    className="w-full h-14 bg-zinc-800 text-white rounded-2xl font-black text-sm flex items-center justify-center hover:bg-zinc-700 active:scale-95 transition-all border border-zinc-700 cursor-pointer"
                >
