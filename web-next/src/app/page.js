...
export default function DashboardPage() {
  ...
  const handleSubmit = async (text) => {
    ...
    router.push(`/chat?role=${role}&initial=${encodeURIComponent(query)}`);
  };
      ...
      <div className="px-4 pt-4">
        ...
        <div className="flex bg-zinc-900 rounded-xl p-1 border border-zinc-800">
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-lg font-bold text-sm transition-all flex items-center justify-center space-x-2 cursor-pointer ${activeTab === "demand"
          ...
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-lg font-bold text-sm transition-all flex items-center justify-center space-x-2 cursor-pointer ${activeTab === "supply"
              ? "bg-white text-black shadow-lg"
              : "text-zinc-400 hover:text-white"
              }`}
          >
          ...
          <>
              ...
              <div className="flex space-x-2 overflow-x-auto scrollbar-hide pb-2">
                {["Haircut", "Cleaning", "Plumbing", "Photography", "Tutoring"].map((service) => (
                  <button
                    ...
                    className="px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-full text-sm font-medium text-zinc-300 hover:bg-zinc-800 hover:border-zinc-700 transition-colors whitespace-nowrap cursor-pointer"
                  >
                    {service}
                  </button>
                ))}
            ...
            <section className="mt-8 space-y-8">
              ...
              <RequestSection
                ...
                onViewAll={() => router.push("/requests?status=pending")}
              />
              ...
              <RequestSection
                ...
                onViewAll={() => router.push("/bookings?status=completed")}
              />
              ...
              {isSignedIn && consumerRequests &&
                ...
                consumerRequests.counts?.completed === 0 && (
                  <div className="text-center py-16 px-6 bg-zinc-900/20 border border-dashed border-zinc-800 rounded-[2rem]">
                    ...
                    <button
                      onClick={() => router.push('/chat?role=consumer')}
                      className="bg-white text-black h-11 px-8 rounded-full font-black text-sm active:scale-95 transition-all cursor-pointer"
                    >
                      Start a Request
                    </button>
                  </div>
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    {enrollmentStatus === "pending_verification" ? (
                      ...
                      <button
                        ...
                        className="w-full h-16 bg-white text-black rounded-2xl font-black text-lg hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-white/10 cursor-pointer"
                      >
                        Become a Provider
                      </button>
                    )}
                    ...
                    <p className="mt-8 text-zinc-600 text-sm font-bold text-center">
                      Already enrolled? <button className="text-blue-500 underline ml-1 cursor-pointer">Sign In</button>
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  {requests.map((req) => (
                    <div
                      key={req.id}
                      onClick={() => router.push(`/provider/request/${req.id}`)}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button className="flex items-center text-xs font-bold text-white bg-white/10 px-3 py-1.5 rounded-full group-hover:bg-green-600 transition-colors cursor-pointer">
          ...
          <div className="flex items-center space-x-3 relative">
            ...
            <div className="flex-1 flex items-center bg-zinc-800 rounded-full border border-zinc-700 pl-5 pr-2 py-2">
              ...
              <div className="flex items-center space-x-1 ml-2">
                <button onClick={() => { setIsListening(true); recognitionRef.current?.start(); }} className="p-2 rounded-full hover:bg-zinc-700 transition-colors cursor-pointer">
                  <Mic size={18} className={isListening ? "text-red-500" : "text-zinc-500"} />
                </button>
                <button onClick={toggleVoiceMode} className="w-9 h-9 bg-white rounded-full flex items-center justify-center hover:bg-zinc-200 transition-colors cursor-pointer">
