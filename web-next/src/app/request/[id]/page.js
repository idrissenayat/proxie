...
export default function RequestDetailsPage() {
    ...
    if (error) return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 text-center">
            ...
            <button onClick={() => router.push('/')} className="px-6 py-3 bg-zinc-900 rounded-2xl font-bold text-white cursor-pointer">Back to Dashboard</button>
        </div>
    );
    ...
    return (
        <div className="flex flex-col min-h-screen bg-black text-white pb-32">
            {/* Header */}
            <header className="fixed top-0 left-0 right-0 z-50 px-4 py-6 bg-black/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-[480px] mx-auto flex items-center justify-between">
                    <button onClick={() => router.push('/')} className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 cursor-pointer">
                        <ChevronLeft size={20} />
                    </button>
                    <h1 className="font-black tracking-tight text-lg">Request Details</h1>
                    <button className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 cursor-pointer">
                        <Share2 size={18} />
                    </button>
                </div>
            </header>
            ...
            <main className="max-w-[480px] mx-auto w-full px-5 pt-28 space-y-8">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${request.status === 'matching' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                            request.status === 'pending' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                                request.status === 'upcoming' ? 'bg-purple-500/10 text-purple-500 border-purple-500/20' :
                ...
                {(canEdit || canCancel) && (
                    <section className="space-y-3">
                        {canEdit && (
                            <button
                                onClick={() => router.push(`/chat?initial=${encodeURIComponent(`I want to edit my request for ${request.service_type}. Here is the current input: ${request.raw_input}`)}`)}
                                className="w-full h-14 bg-zinc-900 border border-white/10 rounded-2xl flex items-center justify-center gap-3 font-bold hover:bg-zinc-800 transition-all cursor-pointer"
                            >
                        ...
                        {canCancel && (
                            <button
                                ...
                                className="w-full h-14 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-center justify-center gap-3 font-bold text-red-500 hover:bg-red-500/10 transition-all cursor-pointer"
                            >
                                <XCircle size={18} />
                                Cancel Request
                            </button>
            ...
            {request.status === 'pending' && (
                <div className="fixed bottom-0 left-0 right-0 p-5 bg-black/80 backdrop-blur-xl border-t border-white/5 z-40">
                    <div className="max-w-[480px] mx-auto">
                        <button
                            onClick={() => router.push(`/request/${id}/offers`)}
                            className="w-full h-16 bg-white text-black rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-2xl shadow-white/10 active:scale-95 transition-all cursor-pointer"
                        >
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 h-14 bg-zinc-800 rounded-2xl font-bold text-white hover:bg-zinc-700 transition-all cursor-pointer"
                            >
                                Nevermind
                            </button>
                            <button
                                ...
                                className="flex-1 h-14 bg-red-500 text-white rounded-2xl font-black hover:bg-red-600 transition-all shadow-lg shadow-red-500/20 disabled:opacity-50 cursor-pointer"
                            >
