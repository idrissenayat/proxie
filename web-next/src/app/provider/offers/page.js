...
import {
    ...
    ChevronRight, MapPin, DollarSign, Briefcase
} from 'lucide-react';
...
export default function ProviderOffersPage() {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                // Mocking data for now as per original component logic
                const mockOffers = [
                    {
                        id: '1',
                        status: 'pending',
        ...
        <div className="flex flex-col min-h-screen bg-black text-white">
            <header className="p-4 pt-6 bg-black/80 backdrop-blur-md sticky top-0 z-40 border-b border-zinc-900/50">
                <div className="flex items-center gap-4 mb-4">
                    <button onClick={() => router.back()} className="text-zinc-500 hover:text-white transition-colors cursor-pointer">
                        <ChevronLeft />
                    </button>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {['all', 'pending', 'accepted', 'declined'].map(f => (
                        <button
                            ...
                            className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider border transition-all cursor-pointer ${activeFilter === f ? 'bg-white text-black border-white' : 'bg-zinc-900 text-zinc-500 border-zinc-800'
                                }`}
                        >
                            {f}
                        </button>
