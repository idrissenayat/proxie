...
import { getProviderProfile, getProviderPortfolio } from '@/lib/api';
...
export default function PublicProviderProfile() {
    ...
    if (!provider) return <div className="min-h-screen bg-black flex items-center justify-center text-red-500 font-bold">Provider not found</div>;
    ...
    return (
        <div className="flex flex-col min-h-screen bg-black text-white pb-32">
            ...
            <div className="relative h-64 md:h-80 bg-zinc-900 overflow-hidden">
                ...
                <button
                    onClick={() => router.back()}
                    className="absolute top-6 left-4 w-10 h-10 bg-black/50 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white z-20 cursor-pointer"
                >
                    <ChevronLeft size={20} />
                </button>
                <button
                    className="absolute top-6 right-4 w-10 h-10 bg-black/50 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white z-20 cursor-pointer"
                >
                    <Share2 size={18} />
                </button>

                <div className="absolute inset-x-0 bottom-0 p-6 z-20 flex flex-col items-center">
                    <div className="w-24 h-24 bg-zinc-800 rounded-3xl border-4 border-black overflow-hidden shadow-2xl mb-4">
                        {provider.profile_photo_url ? (
            ...
            <div className="fixed bottom-0 left-0 right-0 p-5 bg-black/80 backdrop-blur-xl border-t border-white/5 z-40">
                <div className="max-w-[480px] mx-auto">
                    <button
                        onClick={() => router.push(`/chat?role=consumer&initial=${encodeURIComponent(`I'd like to book ${provider.name}`)}`)}
