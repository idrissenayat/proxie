...
import { submitOffer, sendChatMessage } from '@/lib/api';
...
export default function SubmitOfferPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            router.push('/provider?offerSubmitted=true');
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-black text-white pb-10">
            {/* Header */}
            <header className="p-4 pt-6 bg-black sticky top-0 z-40 border-b border-zinc-900">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} className="text-zinc-500 cursor-pointer"><ChevronLeft /></button>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {suggestion?.available_slots?.map((slot, i) => (
                                ...
                                >
                                    <span className="text-[10px] font-black uppercase text-inherit">{new Date(slot.date).toLocaleDateString('en-US', { weekday: 'short' })}</span>
                                    <span className="text-sm font-black text-inherit">{slot.time}</span>
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-3">
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3">
                                ...
                                <input
                                    type="date"
                                    className="bg-transparent border-none outline-none text-white text-sm font-bold w-full cursor-pointer"
                                    value={form.date}
            ...
            <div className="p-5 mt-auto">
                <button
                    ...
                    className="w-full bg-white text-black h-16 rounded-2xl font-black flex items-center justify-center gap-3 disabled:opacity-50 active:scale-95 transition-all shadow-xl shadow-white/5 cursor-pointer"
                >
