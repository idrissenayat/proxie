...
import { getRequest, markLeadViewed } from '@/lib/api';
...
export default function ProviderRequestDetailPage() {
    ...
    if (!request) return <div className="min-h-screen bg-black p-8 text-center text-red-500 font-bold">Lead not found</div>;
    ...
    return (
        <div className="flex flex-col min-h-screen bg-black text-white pb-32">
            ...
            <div className="relative h-72 md:h-96 bg-zinc-900 border-b border-zinc-800">
                ...
                <button
                    onClick={() => router.back()}
                    className="absolute top-6 left-4 w-10 h-10 bg-black/50 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white cursor-pointer"
                >
                    <ChevronLeft size={20} />
                </button>
            </div>
            ...
            {/* Bottom Actions */}
            <div className="fixed bottom-0 left-0 right-0 p-5 bg-black/80 backdrop-blur-xl border-t border-zinc-900 z-40">
                <div className="max-w-2xl mx-auto flex gap-3">
                    <button
                        onClick={() => router.push(`/chat?role=provider&request_id=${id}`)}
                        className="flex-1 bg-zinc-900 border border-zinc-800 text-zinc-300 h-14 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 transition-all cursor-pointer"
                    >
                        <MessageSquare size={18} /> Ask Agent
                    </button>
                    <button
                        onClick={() => router.push(`/chat?role=provider&initial=${encodeURIComponent(`I'd like to make an offer for the ${request.service_type} lead in ${request.location?.city}`)}`)}
                        className="flex-[2] bg-white text-black h-14 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-zinc-200 transition-all shadow-lg shadow-white/5 active:scale-95 cursor-pointer"
                    >
