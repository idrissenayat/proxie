...
import {
    ...
    ImageIcon, PlayCircle, Plus, Mic, AudioWaveform
} from 'lucide-react';
import { getProviders, getRequests } from '../api/client';
...
const ProviderDashboardPage = () => {
    ...
    const stats = [
        ...
        { label: 'Upcoming', value: '5', icon: CheckCircle2, color: 'text-green-400', bg: 'bg-green-400/10', path: '/provider/offers?filter=accepted' },
    ];
    ...
    return (
        <div className="flex flex-col min-h-screen bg-zinc-950 text-white font-sans selection:bg-blue-500/30">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center border border-zinc-800"
                        >
                            <User size={18} className="text-zinc-400" />
                        </button>
                        <button className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center border border-zinc-800 relative">
                            <Bell size={18} className="text-zinc-400" />
                            {requests.length > 0 && (
                                <span className="absolute top-2 right-2 w-2 h-2 bg-blue-500 rounded-full border-2 border-zinc-950 shadow-lg shadow-blue-500/20"></span>
                            )}
                        </button>
                    </div>
                </div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                    {providers.map(p => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all whitespace-nowrap ${selectedProvider?.id === p.id
            ...
            <main className="flex-1 overflow-y-auto px-4 pb-32">
                ...
                <section className="mb-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-lg text-xs font-bold text-zinc-400">
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-zinc-950 via-zinc-950 points-transparent pt-8 z-40">
                <div className="max-w-[480px] mx-auto">
                    <div className="flex items-center space-x-3">
                        <button
                            onClick={() => navigate('/chat?role=provider')}
                            className="w-12 h-12 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center hover:bg-zinc-800 shrink-0 shadow-lg"
                        >
                        ...
                        <div
                            onClick={() => navigate('/chat?role=provider')}
                            className="flex-1 flex items-center bg-zinc-900/90 backdrop-blur-md rounded-full border border-zinc-800 pl-5 pr-2 py-2 cursor-text"
                        >
...
const LeadCard = ({ lead, selectedProviderId }) => {
    const navigate = useNavigate();
    const isNew = !lead.viewed_by_current_provider;

    return (
        <div
            onClick={() => navigate(`/provider/request/${lead.id}`, { state: { providerId: selectedProviderId } })}
        ...
        >
            <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-green-400 text-sm font-black flex items-center">
                    <DollarSign size={14} /> {lead.budget?.min}-{lead.budget?.max}
                </div>
            </div>
                ...
                <div className="flex gap-2 mb-4">
                    {lead.media.slice(0, 3).map((m, i) => (
                        <div key={i} className="w-12 h-12 rounded-lg bg-zinc-800 overflow-hidden border border-zinc-800">
                            <img src={m.preview || m.url} className="w-full h-full object-cover" />
                        </div>
                    ))}
                    {lead.media.length > 3 && (
