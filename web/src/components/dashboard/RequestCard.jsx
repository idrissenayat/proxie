            ...
            return (
                <div className="space-y-3">
                    ...
                    <div className="flex gap-2 pt-1">
                        <button className="flex-1 bg-zinc-800 hover:bg-zinc-700 h-9 rounded-lg text-xs font-bold transition-colors">
                            Message
                        </button>
                        <button
                            onClick={() => navigate(`/booking/${data.booking_id}`)}
                            className="flex-1 bg-white text-black h-9 rounded-lg text-xs font-bold transition-colors"
                        >
                            Details
                        </button>
                    </div>
                ...
                <div className="space-y-3">
                    ...
                    {!data.has_review && (
                        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-3 flex items-center justify-between">
                            ...
                            <button className="text-[11px] font-black text-white hover:underline">REVIEW</button>
                        </div>
                    )}
                    ...
                    <div className="pt-1">
                        <button
                            onClick={() => navigate('/chat', { state: { rebook: data.booking_id } })}
                            className="w-full bg-zinc-800 hover:bg-zinc-700 h-10 rounded-xl text-xs font-black uppercase tracking-widest transition-colors"
                        >
                            Book Again
                        </button>
                    </div>
        ...
        return (
            <div className="space-y-4">
                <div
                    ...
                    onClick={() => navigate(`/request/${data.id || data.booking_id}`)}
                >
                ...
                {type === 'open' ? (
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between py-2 px-3 bg-blue-500/5 border border-blue-500/10 rounded-xl">
                            ...
                            <button
                                onClick={(e) => { e.stopPropagation(); navigate(`/offers?request_id=${data.id}`); }}
                                className="text-white text-[10px] font-black underline decoration-blue-500/50 underline-offset-4"
                            >
                                VIEW ALL
                            </button>
                        </div>
                ...
                <div className="flex gap-2">
                    <button
                        onClick={(e) => { e.stopPropagation(); navigate('/chat', { state: { request_id: data.id } }); }}
                        className="flex-1 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 h-10 rounded-xl text-xs font-bold text-zinc-400 flex items-center justify-center gap-2 transition-all"
                    >
                    ...
                    {type === 'pending' && (
                        <button
                            onClick={(e) => { e.stopPropagation(); navigate(`/offers?request_id=${data.id}`); }}
                            className="flex-1 bg-white text-black h-10 rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1 shadow-lg shadow-white/5"
                        >
